...
migrateup:
	migrate -path db/migration -database "postgresql://root:postgres@postgres15:5432/simple_bank?sslmode=disable" -verbose up
...
.PHONY: postgres createdb dropdb migrateup migratedown sqlc test startpostgrescontainer run mock migratedown1 migrateup1
